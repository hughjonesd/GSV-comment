---
title: "Lies, versus the lying liars who tell them: comment on Garbarino, Slonim and Villeval (2018)"
...
Some people are honest, while others will lie whenever it benefits them. We would like to know the
prevalence of lying, since a society where many people lie is likely to experience
economic and social problems. Since we cannot simply ask people if they are liars, one way to
estimate the proportion of liars in a group is to ask them to report the result of a coin flip or
other random device, offering them a payment if they report heads. Liars don't always lie: they 
only lie when it benefits them. So they always report heads irrespective of the true coin flip.
^[@Garbarino2018 maintain this assumption and so shall I.] If there are many more heads than
we would expect by chance, we can assume many people are lying. But how many?
...
@Garbarino2018 -- GSV from here on -- introduce an alternative method. They claim that their method 
corrects for this problem and can estimate the full distribution of
...
sample_from_pop <- TRUE # if TRUE, the number of liars in the sample is randomly drawn 
                        # from a population with `Liars` liars. 
                        # If FALSE it is fixed at `Liars * N`.
estimate_lies_not_liars <- FALSE # if FALSE, use Liars/N as ground truth
...
results_R <- results[1, , ]
results_heads <- results[2, , ]
colnames(results_R) <- paste0("R", 1:nreps)
colnames(results_heads) <- paste0("heads", 1:nreps)

sim_data <- true_data %>% 
      cbind(as_tibble(results_R)) %>% 
      cbind(as_tibble(results_heads)) %>% 
      gather("stat", "result", starts_with("R"), starts_with("heads")) %>%
      extract(stat, c("stat", "sim"), regex="(.*?)(\\d+)", convert = TRUE) %>%
      spread(stat, result) %>%  
      arrange(N, Liars, P)
...
# alternative "ground truth" is proportion of lies: (R - heads)/(N - heads)
