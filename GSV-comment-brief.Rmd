...
and intervals fail even for (R-T)/(N-T). Convert to sampling from a cts distrib.
...
file_root <- paste0("GSV-sims", if (sample_from_pop) "-pop-sample")
sim_output_file <- paste0(file_root, "Output.csv")
if (rerun_java) {
  sim_input_file <- paste0(file_root, ".csv")
  readr::write_csv(sim_output, path = sim_input_file)
...
```{r country-comparisons, eval = FALSE}
...
cc_hux <- as_huxtable(ctry_comp, add_colnames = TRUE, add_rownames = TRUE) %>% 
      set_caption("Bayesian country comparisons. 
            Cells give the probability that row country has higher \\lambda than column country.") %>% 
