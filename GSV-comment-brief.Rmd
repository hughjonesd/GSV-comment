...
sim_output <- sim_data %>% 
  ...
  path_root <- paste0("GSV-sims", if (sample_from_pop) "-pop-sample")

if (rewrite_csv) {
  readr::write_csv(sim_output, path = paste0(path_root, ".csv"))
...
output_path <- paste0(path_root, "Output.csv")
gsv_estimates <- suppressMessages(readr::read_csv(normalizePath(paste0("~/", output_path))))
...
is substantial shrinkage from the "naïve" per-country estimates found by calculating *2R/N - 1*
