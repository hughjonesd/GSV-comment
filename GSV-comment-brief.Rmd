...
>  of the number of liars in "coin flip" style experiments. However, their method appears 
>  to give erroneous confidence intervals. I explain why, and describe two better
...
huxtable::set_default_properties(position = "left", top_padding = 7, bottom_padding = 12)
set.seed(19751027)
...
$\textit{Lies}$ is indeed 100%.  But it is 100% of 1 person
-- the only person who saw tails. That one person's behaviour is an unbiased estimate of the
proportion of liars in the sample, $\textit{Liars}$. But it is a very noisy estimate.

The first problem means that when $R$ is very high or low,
the percentage of confidence intervals containing $\textit{Lies}$
may fall off sharply.^[Depending on the distribution of the number of liars $L$.]

The second problem means that the percentage of confidence intervals containing $\textit{Liars}$
...
One can then estimate the proportion of $\textit{Liars}$ as $L/N$. From
this, one can derive confidence intervals and expected values in the usual way.^[R code to
run these calculations, along with all the code for this article, is available at https://github.com/hughjonesd/GSV-comment .]

For the estimation here, I used a uniform prior, with $Pr(L) = \frac{1}{N+1}$ for $L$ in  0
to $N$.

Results are shown in Table \ref{tab:coverage-allmethods}. Both frequentist and  Bayesian methods
mostly achieve the nominal confidence level, with more than 90/95/99% of intervals containing the
true value of $Liars$. The exception is the frequentist 99% confidence interval, which is too
narrow.
...
as_huxtable(coverage_N, add_colnames = TRUE) %>% 
      ...
      theme_article(header_col = FALSE) %>% 
      set_bottom_border(6, 1, 0.4) # rowspan problems
```
...
We can also compare the accuracy of point estimates of $\textit{Liars}$. Table
...
```{r hj2016-pic, fig.cap = "Posterior means and confidence intervals for proportion dishonest in 15 countries. Crosses are naïve estimates.", fig.align="center"}
...
we will end up believing that the British people in the sample are less honest, and the Chinese 
people more honest, than the experimental data *per se* suggest.
...
This approach has a nice interpretation, somewhat analogous
to the use of the null hypothesis in frequentist methods. We start from the 
belief that the proportion of liars is the same across all groups. The posterior
then shows how much the evidence changes this belief.
...
So far we have been trying to estimate the proportion of $\textit{Liars}$ in the sample. In fact,
we usually care about the proportion in liars in populations from which the sample is drawn. This
...
simulations, not shown, suggest that Bayesian confidence intervals are less robust in bounding
$Liars$ in the population from which the sample $L$ is randomly drawn.
...
3. If your N is small and you are interested in the sample proportion of $\textit{Liars}$, consider
...
$N$ balls, of which $T$ are white. Pay them if they report a white ball. You will then know that the 
proportion of lies told is $(R-T)/(N-T)$, and this will also be the proportion of liars, in a sample
of $N-T$ subjects. So you can estimate the proportion of $\textit{Lies}$, and of the lying 
$\textit{Liars}$ who tell them.
...
```{r coverage-pic, fig.cap = "GSV confidence interval coverage by confidence level, N and P", fig.align="centre"}
...
```{r coverage-grp-pic, fig.cap = "GSV confidence interval coverage by confidence level, N, P and true proportion of liars", fig.align="centre"}
...
true proportion of liars in the sample (True $L/N$), for $N$ of 10 and 50. The Bayesian method is best 
for all values of $L/N$ up to 80%.

```{r errors-by-true-prop, fig.cap = "Errors by method and true proportion of liars, N = 10 and 50", fig.align="centre"}
...
g <- ggplot(errors_true_prop, aes(`True L/N`, mean_sq_error, colour = Method)) + 
...
```{r errors-disagg, include = FALSE, fig.align="centre", fig.cap="Point estimate errors by R/N, N and P"}
